...
class backup_randomstrayquotes_activity_structure_step extends backup_activity_structure_step {
    ...
    protected function define_structure() {
        ...
        $newmodule = new backup_nested_element('randomstrayquotes', array('id'), array(
            'name', 'intro', 'introformat', 'grade'));

        // If we had more elements, we would build the tree here.

        // Define data sources.
        $newmodule->set_source_table('randomstrayquotes', array('id' => backup::VAR_ACTIVITYID));

        // If we were referring to other tables, we would annotate the relation
        // with the element's annotate_ids() method.

        // Define file annotations (we do not use itemid in this example).
        $newmodule->annotate_files('mod_randomstrayquotes', 'intro', null);
