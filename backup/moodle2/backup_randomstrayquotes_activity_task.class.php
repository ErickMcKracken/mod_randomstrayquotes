...
class backup_randomstrayquotes_activity_task extends backup_activity_task {
    ...
    static public function encode_content_links($content) {
        ...
        // Link to the list of randomstrayquotes.
        $search = '/('.$base.'\/mod\/randomstrayquotes\/index.php\?id\=)([0-9]+)/';
        $content = preg_replace($search, '$@RANDOMSTRAYQUOTESINDEX*$2@$', $content);

        // Link to randomstrayquotes view by moduleid.
        $search = '/('.$base.'\/mod\/randomstrayquotes\/view.php\?id\=)([0-9]+)/';
        $content = preg_replace($search, '$@RANDOMSTRAYQUOTESVIEWBYID*$2@$', $content);
